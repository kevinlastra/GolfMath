...
void ToutEnUn::mousePressEvent(QMouseEvent *event)
{
    ...
    if (joueurIA == 1 && i > nombreJoueur - 1)
    {
...
void ToutEnUn::DeplacementIA(int p)
{
    ...
    case 2:
    {
        ...
        if (T->getNode(posj2)->getType() == Node::end)
        {
            finij2 = true;
            QMessageBox::information(this, "Bravo !!", "Joueur 2 a terminé en " + QString::number(pointj2) + " coups.");
        }
    ...
    case 3:
    {
        IADeplacement(T, noeud, manche, posj3);
        if (tour == 1)
        {
            posj3.x = posj2.x;
            posj3.y = posj2.y;
        }
        if (T->getNode(posj3)->getType() == Node::end)
        {
            finij3 = true;
            QMessageBox::information(this, "Bravo !!", "Joueur 3 a terminé en " + QString::number(pointj3) + " coups.");
        }
        jeu->addWidget(j3, posj3.x, posj3.y);
        j3->update();
        jeu->update();
        break;
    }
    case 4:
    {
        IADeplacement(T, noeud, manche, posj4);
        if (tour == 1)
        {
            posj4.x = posj3.x;
            posj4.y = posj3.y;
        }
        if (T->getNode(posj4)->getType() == Node::end)
        {
            finij4 = true;
            QMessageBox::information(this, "Bravo !!", "Joueur 4 a terminé en " + QString::number(pointj4) + " coups.");
        }
        jeu->addWidget(j4, posj4.x, posj4.y);
        j4->update();
        jeu->update();
        break;
    }
